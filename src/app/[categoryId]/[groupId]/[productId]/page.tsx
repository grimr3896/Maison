...
export default async function ProductPage({ params }: { params: Promise<{ categoryId: string, groupId: string, productId: string }> }) {
  ...
  if (!result || result.category.id !== categoryId || Object.keys(result.category.groups).find(k => k === groupId) === undefined) {
     // Validate that the product exists and the path parameters match the catalog data
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-6">
              ...
              <p className="text-center text-[0.65rem] text-muted-foreground tracking-wide font-light">
                No payment needed now — our team will reach out with a personalized quote and delivery plan.
              </p>
            </div>
          </div>
        </div>
